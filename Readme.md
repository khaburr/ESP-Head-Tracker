...
To configure it just set network name, network password, and target PC ip address, switch on device, let it sit flat for about 5 seconds, and open Opentrack with Input set to FreePIE UDP reciver. I've created drift compensation brackets that assume that most of the time you are facing the monitor, it works good for DCS.
